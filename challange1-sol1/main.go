...
func SubTotalPaket(area string, data KurirGroup, resultChannel chan int) {
	subTotal := 0
	for _, v := range data {
		subTotal += v.paket
	}
...
func calcTotalPaket(kurirGroupArea KurirGroupArea) {
	// declare channel
	channel := make(chan int, 2)

	for area, group := range kurirGroupArea {

		go SubTotalPaket(area, group, channel)
	}
